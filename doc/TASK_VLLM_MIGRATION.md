...
- [ ] `generate_with_vllm()` — `POST /v1/completions` 호출
  - Ollama `options.num_predict` → vLLM `max_tokens`
  - 응답: `result["choices"][0]["text"]`
- [ ] `chat_with_vllm()` — `POST /v1/chat/completions` 호출
  - 파라미터 평탄화 (temperature, max_tokens, top_p, stop 최상위)
  - 응답: `result["choices"][0]["message"]["content"]`
- [ ] `chat_with_vllm_stream()` — SSE 스트리밍 파싱
  - `data: ` 접두사 제거 → JSON 파싱
  - `choices[0].delta.content` 에서 토큰 추출
  - `[DONE]` 또는 `finish_reason != null` 시 종료
- [ ] Circuit Breaker 기존 `_breaker` 인스턴스 재사용

### Step 2: 유틸리티 함수 작성

- [ ] `check_vllm_health()` — `GET /health` → bool 반환
- [ ] `list_models()` — `GET /v1/models` → 모델명 리스트 반환

### Step 3: 단위 검증

- [ ] `chat_with_vllm()` 직접 호출 → 정상 응답 확인
- [ ] `chat_with_vllm_stream()` 호출 → 토큰 단위 수신 확인
- [ ] `check_vllm_health()` → True 반환 확인
...
- [ ] `vllm_base_url: str = "http://192.168.0.22:8000"` 추가
- [ ] `vllm_model: str = "qwen2.5-7b"` 추가
- [ ] `llm_backend` 기본값을 `"vllm"`으로 변경, 설명에 `"huggingface | ollama | vllm"` 명시

### Step 2: 기존 설정 유지 확인

- [ ] Ollama 설정(`ollama_base_url`, `ollama_model`, `ollama_embed_model`) 그대로 유지 (폴백 + 임베딩용)
- [ ] `llm_backend` 필드 description 업데이트

### Step 3: 검증

- [ ] `LLM_BACKEND=vllm` 환경변수 → `settings.llm_backend == "vllm"` 확인
- [ ] `LLM_BACKEND=ollama` 환경변수 → 기존 Ollama 설정 정상 로드 확인
...
- [ ] `infer()` — `llm_backend == "vllm"` 시 `generate_with_vllm()` 호출
- [ ] 기존 `ollama` / `huggingface` 분기 유지

### Step 2: /infer/medical 엔드포인트 분기

- [ ] `infer_medical()` — `llm_backend == "vllm"` 시 `chat_with_vllm()` 호출
- [ ] 기존 Ollama Chat 호출 분기 유지

### Step 3: /infer/medical/stream 엔드포인트 분기

- [ ] `infer_medical_stream()` — `llm_backend == "vllm"` 시 `chat_with_vllm_stream()` 호출
- [ ] SSE 생성 로직에서 vLLM 스트리밍 형식 처리
  - vLLM: `item["token"]` (vllm_service에서 통일된 형식으로 반환)

### Step 4: /infer/rule 엔드포인트 분기

- [ ] `infer_rule()` — `llm_backend == "vllm"` 시 `chat_with_vllm()` 호출

### Step 5: /health 헬스체크 수정

- [ ] `llm_backend == "vllm"` 시 `check_vllm_health()` 호출
- [ ] Circuit Breaker 상태는 `vllm_service._breaker` 참조

### Step 6: 통합 검증

- [ ] `LLM_BACKEND=vllm` 상태에서 4개 엔드포인트 모두 정상 동작
...
- [ ] `LLM_BACKEND=vllm` 으로 변경
- [ ] `VLLM_BASE_URL=http://192.168.0.22:8000` 추가
- [ ] `VLLM_MODEL=qwen2.5-7b` 추가
- [ ] 기존 `OLLAMA_BASE_URL` 유지 (임베딩 + 폴백용)
