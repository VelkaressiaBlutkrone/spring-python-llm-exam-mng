...
| #   | 항목                             | 효과          | 난이도    | 상태        |
| --- | -------------------------------- | ------------- | --------- | ----------- |
| 1   | httpx 공유 클라이언트 전환 (1.1) | 레이턴시 감소 | 낮음      | ✅ 구현 완료 |
| 2   | Ollama 호출 중복 제거 (1.2)      | 유지보수성    | 낮음      | ✅ 구현 완료 |
| 3   | 벡터/FULLTEXT 병렬 검색 (2.2)    | 응답속도 40%↑ | 낮음      | ⬜ 미구현    |
| 4   | response_cleaner 주석 수정 (5.1) | 가독성        | 매우 낮음 | ✅ 구현 완료 |
| 5   | typo_corrector 정리 (5.2)        | 가독성        | 매우 낮음 | ✅ 구현 완료 |
| 6   | Health Check 강화 (6.4)          | 운영 안정성   | 낮음      | ✅ 구현 완료 |

### Phase 2 — 단기 개선 (1-2주)

| #   | 항목                            | 효과            | 난이도 | 상태        |
| --- | ------------------------------- | --------------- | ------ | ----------- |
| 7   | 임베딩 캐시 도입 (2.1)          | 반복 쿼리 속도↑ | 낮음   | ⬜ 미구현    |
| 8   | ChromaDB 컬렉션 분리 (2.3)      | 검색 정확도↑    | 중간   | ⬜ 미구현    |
| 9   | 프롬프트 외부화 (1.3)           | 이터레이션↑     | 낮음   | ✅ 구현 완료 |
| 10  | Few-shot 예시 추가 (4.1)        | 응답 품질↑      | 낮음   | ⬜ 미구현    |
| 11  | Circuit Breaker (6.1)           | 장애 대응↑      | 중간   | ✅ 구현 완료 |
| 12  | 테스트 커버리지 강화 (5.4)      | 안정성↑         | 중간   | ✅ 부분 구현 |
| 13  | 입력 길이/CORS/Rate Limit (7.x) | 보안↑           | 낮음   | ✅ 구현 완료 |

### Phase 3 — 중기 고도화 (3-4주)

| #   | 항목                            | 효과          | 난이도 | 상태        |
| --- | ------------------------------- | ------------- | ------ | ----------- |
| 14  | Re-ranking 도입 (3.1)           | RAG 품질↑↑    | 높음   | ✅ 구현 완료 |
| 15  | 쿼리 확장 (3.2)                 | 검색 재현율↑  | 중간   | ✅ 구현 완료 |
| 16  | Chunk 전략 개선 (3.4)           | 긴 문서 검색↑ | 중간   | ✅ 구현 완료 |
| 17  | 대화 이력 지원 (4.2)            | 사용자 경험↑↑ | 높음   | ✅ 구현 완료 |
| 18  | 구조화 로깅 + 메트릭 (6.2, 6.3) | 운영 가시성↑  | 중간   | ✅ 메트릭 구현, 구조화 로깅 미구현 |
| 19  | 증분 인덱싱 (2.4)               | 운영 효율↑    | 중간   | ⬜ 미구현    |

### Phase 4 — 장기 (1-2개월)

| #   | 항목                             | 효과        | 난이도 | 상태        |
| --- | -------------------------------- | ----------- | ------ | ----------- |
| 20  | 오타 사전 DB화 + 자동 확장 (4.3) | 커버리지↑   | 높음   | ✅ DB화 완료, 자동 확장 미구현 |
| 21  | 응답 품질 피드백 루프 (4.4)      | 지속 개선   | 높음   | ✅ 피드백 API 구현 완료 |
| 22  | 의존성 핀닝 + CI (5.3)           | 빌드 안정성 | 중간   | ⬜ 미구현    |
